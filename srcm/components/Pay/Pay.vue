<template>
  <div id="pay">
    ...
    <div class="card-tit"  v-if="payway=='yve'">
      ...
      <p><b>本次支付金额：</b><i v-text="mymoney>=needpaymoney? needpaymoney:mymoney"></i></p>
        ...
        if (this.yvetime <= 60) {
          ...
          if (this.mymoney > 0) {
            ...
            axios.post(BASE_URL + '/index.php?r=weiXinPay/send', qs.stringify({
              ...
              payMoney: this.mymoney>=this.needpaymoney? this.needpaymoney:this.mymoney,
              token: this.token
            })).then(function (res) {
              Indicator.close()
    ...
    mounted(){
      ...
      if(!myopenid){
        if(url.indexOf('openid=')=='-1'){
          ...
          window.location.href='https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxb68ed0995576d589&redirect_uri='+gohref+'&response_type=code&scope=snsapi_base&state=123#wechat_redirect';	//测试appid
        }else{
